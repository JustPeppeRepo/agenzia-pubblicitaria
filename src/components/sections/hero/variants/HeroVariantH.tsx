...
function OrbitTrack({
  ...
  return (
    <div className={cn("absolute inset-0 flex items-center justify-center", offset)}>
      {/* Shared box: SVG stroke + offset-path use the same ellipse */}
      <div
...
/** Orbits sized from the mockup box so clearance scales with viewport. */
...
function ProductMockup({ live }: { live: boolean }) {
  return (
    <div className="relative mx-auto w-full max-w-[440px] md:max-w-none">
      <StackOrbits live={live} />

      {/* Soft bloom — oklab stops + blur so edges feather into the page bg */}
      <div
        aria-hidden
        className="pointer-events-none absolute -inset-[35%] z-0"
        style={{
          background: [
            "radial-gradient(ellipse 82% 68% at 60% 32%, color-mix(in oklab, var(--accent) 38%, transparent) 0%, color-mix(in oklab, var(--accent) 16%, transparent) 34%, color-mix(in oklab, var(--accent) 5%, transparent) 58%, transparent 82%)",
            "radial-gradient(ellipse 58% 52% at 36% 64%, color-mix(in oklab, var(--accent-2) 22%, transparent) 0%, color-mix(in oklab, var(--accent-2) 7%, transparent) 42%, transparent 74%)",
          ].join(", "),
          filter: "blur(36px)",
          opacity: 0.9,
        }}
      ...
      {/* Parent carries perspective; child rotates — avoids transform clash with float */}
      <div
        className={cn("hero-h-stage relative z-20", live && "hero-float-c")}
      >
